...
async function searchGreenhouseJobs() {
  ...
  for (const co of CONFIG.greenhouseCompanies) {
    try {
      const res = await fetch(`https://boards.greenhouse.io/api/v1/boards/${co.slug}/jobs?content=true`, {
        headers: { 'User-Agent': 'Mozilla/5.0 (compatible; JobBot/1.0)' }
      });
      if (!res.ok) { console.log(`[gh] ${co.name}: ${res.status}`); continue; }
      const data = await res.json();
      const jobs = (data.jobs || []).filter(j => isAERole(j.title));
      console.log(`[gh] ${co.name}: ${jobs.length} AE roles`);
      for (const j of jobs) {
        const location = j.location?.name || 'Remote';
        if (!isRemoteFriendly(location)) continue;
        results.push({ jobId: `gh-${j.id}`, source: 'Greenhouse', title: j.title, company: co.name, location, salary: extractSalary(j.content||''), applyUrl: j.absolute_url||`https://boards.greenhouse.io/${co.slug}/jobs/${j.id}`, postedDate: j.updated_at ? new Date(j.updated_at) : new Date() });
      }
    } catch (err) { console.error(`[gh] ${co.name}:`, err.message); }
  }
  return results;
}

// ── SOURCE 3: Google ATS sweep ────────────────────────────────────────────────
...
function parseGoogleResults(message) {
  const jobs = [];
  ...
  for (const line of text.split('\n')) {
    ...
    const source = url.includes('greenhouse') ? 'Greenhouse' : url.includes('lever.co') ? 'Lever' : url.includes('ashby') ? 'Ashby' : 'ATS';
